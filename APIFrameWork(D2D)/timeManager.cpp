...
void timeManager::render(ID2D1RenderTarget* RenderTarget)
{
	...
	if (_timer != nullptr)
	{
		...
		D2TextOut(RenderTarget, str, TextFormat, D2D1::RectF(0, 0,200, 0), Brush);
		swprintf_s(str, TEXT("worldTime: %f"), _timer->getWorldTime());
		D2TextOut(RenderTarget, str, TextFormat, D2D1::RectF(0, 15,200, 15), Brush);
		swprintf_s(str, TEXT("ElapsedTime: %f"), _timer->getElapsedTime());
		D2TextOut(RenderTarget, str, TextFormat, D2D1::RectF(0, 30, 200, 30), Brush);
	}
#else
	if (_timer != nullptr)
	{
